...
struct Seat {
    bool isAvailable;
    string type;      // Window, Middle, Aisle
    string classType; // Business, Premium Economy, Economy
    double price;
};
...
public:
    Flight(string airline, string flightNo, string dest, int seats, double price,
           string deptime, string arrtime, int operationaldays)
        : airline(airline), flightNumber(flightNo), destination(dest), totalSeats(seats),
          availableSeats(seats), basePrice(price), departuretime(deptime),
          arrivaltime(arrtime), operationaldays(operationaldays), startdate() {
    ...
    bool isDateWithinOperationalPeriod(const string& dateOfJourney) {
        ...
        if (journeyDate < startdate) return false;
    ...
    double getSeatPrice(int seatNumber) {
        if (seatNumber >= 1 && seatNumber <= totalSeats) {
            return seats[seatNumber - 1].price;
        }
        return -1;
    }
    ...
    static bool isValidSeatType(const string& type) {
        static unordered_set<string> validTypes = {"Window", "Middle", "Aisle"};
        return validTypes.count(type) > 0;
    }

    static bool isValidSeatClass(const string& classType) {
        static unordered_set<string> validClasses = {"Business", "Premium Economy", "Economy"};
...
public:
    ...
    void bookflight(FlightBase* flight, int seatNum, const string& seatClass, const string& dateofjourney) {
        ...
        if (realFlight->bookseat(seatNum)) {
            ...
            cout << "Booking Price: Rs:" << realFlight->getSeatPrice(seatNum) << endl;
        } else {
            cout << "Sorry, the seat " << seatNum << " is unavailable.\n";
        }
    }
};
...
public:
    ...
    void bookflightforpassenger(Passenger& passenger, string flightNo, int seatNum,
                                string seatClass, string dateofjourney) {
        for (FlightBase* flight : flights) {
    ...
    vector<FlightBase*> getFlights() { return flights; }
};
...
int main() {
    ...
    while (choice != 4) {
        ...
        if (choice == 1) {
            ...
            string name, flightNo, dateofjourney, destination;
            int seatNum;
